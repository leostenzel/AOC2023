...
# ╔═╡ b5d43774-0449-4741-95c9-d85e70351a96
using Graphs, GraphMakie, CairoMakie, Multigraphs, DataStructures

# ╔═╡ 39249de6-a2ef-11ee-02ce-adf6f87a1e37
input = "input" |> readlines

# ╔═╡ 7e0379a0-5fba-49ed-883e-53b3aef0b3e6
nodes = (input .|> x->replace(x, ':' => "") |> 
	x->split(x, ' ') |> Set) |> 
	x-> reduce(∪, x)

# ╔═╡ cc13ba9e-104a-4608-b974-a49f4fe78773
n_dict = Dict(k=>v for (v,k) ∈ enumerate(nodes))

# ╔═╡ 97500b91-9b0e-44e7-9a9a-d8c8b7a43aba
g = let 
	g = SimpleGraph(length(nodes))
	for line ∈ input
		from, to = split(line, ':') .|> x->split(x, ' ', keepempty=false)
		add_edge!.((g,), (n_dict[from[]],), getindex.((n_dict,), to))
	end
	g
end

# ╔═╡ 5443d0cf-1688-48f3-ba5e-50944e996a4a
md"""
Now, the simplest solution is just plotting the graph, because `GraphMakie` does the job of finding the min cut for us 😂
"""

# ╔═╡ 677c144c-6e8f-4b95-be88-212f0fd43295
graphplot(g, elabels=string.(1:length(edges(g))))

# ╔═╡ 5f895e51-c190-45e6-89f4-326bd569d6a5
md"""Of course, your demo input will probably have different edge indices…"""

# ╔═╡ 275903d1-47a5-4890-b780-e3f367c894a7
g2 = let
	g2 = copy(g)
	e = g2 |> edges |> collect
...
# ╔═╡ a517a829-f0ee-473e-bbe2-5942996ed051
connected_components(g2) .|> length |> prod

# ╔═╡ c6682570-b9a5-48a2-97f8-052a7cfc002f
md"""For a more general approach, we pick edges at random and contract them—which is my basic understanding of Karger's algorithm.

We need to keep track of the edges' multiplicity to make sure we found the correct solution in the end. `Multigraphs` can do this for us, but we need to implement `merge_vertices!` for `Multigraphs` ourselves:
"""

# ╔═╡ f9c13f37-41cb-462f-bb72-65d55df6002c
function Graphs.merge_vertices!(g::AbstractMultigraph{T}, a::T, b::T) where {T}
	for c ∈ neighbors(g, b)
		m = mul(g, b, c)
		c == a && continue
		add_edge!(g, a, c, m)
...
# ╔═╡ c7eff922-2a0d-4470-86b6-8a7cd70291d2
md"""A single iteration contracts the graph until only two vertices are left:"""

# ╔═╡ 1ff7056a-ecaf-4a8f-b5e5-228db00b63b6
function karger_iteration(g::AbstractGraph)
	g = Multigraph(g)
	s_dict = DefaultDict{Int, Int}(1)
	while length(vertices(g)) > 2
		a = rand(vertices(g))
		b = rand(neighbors(g, a))
		merge_vertices!(g, a, b)
		s_dict[a] += s_dict[b]
	end
	g, getindex.((s_dict,), vertices(g))
end

# ╔═╡ a0f6edd7-1e8d-49e8-948a-948c32ef7b5f
md"And we repeat the process until the multiplicity of the edge is below the desired threshold:"

# ╔═╡ c299d19c-1673-472c-87b9-4bbaf2b70542
function karger(g::AbstractGraph, cut_size::Integer=3; max_iteration::Integer=10_000) 
	for _ ∈ 1:max_iteration
		cut, sizes = karger_iteration(g)
		mul.(edges(cut))[] <= cut_size && return cut, sizes
	end
	error()
end

# ╔═╡ 1c0f641e-6325-4b4a-a02e-24fe91af1fdb
md"This random algorithm seems pretty slow. Often takes multiple seconds on my Apple M1"

# ╔═╡ 024f0411-c754-4950-b922-403d3d0e672f
cut, sizes = karger(g)

# ╔═╡ 19422454-2611-4755-8c40-5a2910a9b867
prod(sizes)
...
manifest_format = "2.0"
project_hash = "74fde9d63c195c2734988e691b18cb890d307aa8"
